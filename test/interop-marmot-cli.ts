...
async function main() {
  ...
  if (!events.length) {
    console.log('❌ No KP found');
    process.exit(1);
  }
  const kaiEvent = events[0];
  console.log(`   ✅ Event: ${kaiEvent.id.slice(0, 16)}...`);
  console.log(
    `   Client: ${kaiEvent.tags.find((t) => t[0] === 'client')?.[1] || 'unknown'}`
  );
  console.log(
    `   Ciphersuite: ${kaiEvent.tags.find((t) => t[0] === 'mls_ciphersuite')?.[1]}`
  );
  ...
  console.log(
    `   ✅ Parsed: ${rawBytes.length} bytes, ciphersuite=${kaiResult.parsed.ciphersuite}`
  );
  console.log(
    `   Header: ${Array.from(rawBytes.slice(0, 4))
  ...
  const gidHex = Array.from(crypto.getRandomValues(new Uint8Array(16)))
  ...
  console.log(
    `   Exporter: ${Array.from(group.exporterSecret.slice(0, 8))
      .map((b) => b.toString(16).padStart(2, '0'))
      .join('')}...`
  );
  ...
  console.log(
    `   Exporter: ${Array.from(addResult.exporterSecret.slice(0, 8))
      .map((b) => b.toString(16).padStart(2, '0'))
      .join('')}...`
  );
  ...
  console.log(
    `   Header: ${Array.from(welcomeBytes.slice(0, 4))
      .map((b) => '0x' + b.toString(16).padStart(2, '0'))
      .join(' ')}`
  );
